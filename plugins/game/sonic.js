 ...
 * 📌 fitur : [ sonic - سونيك ]
 * 🏷 type : Plugin ESM
 * 💡 info : Speedy Dash (Sonic Runner) Interactive Game
...
html,body{width:100%;direction:rtl}
...
.tt{font:900 17px 'Arial Black';color:#58c7ff;text-shadow:0 0 12px #58c7ff66;letter-spacing:1px}
.tt small{display:block;font:700 7.5px Arial;letter-spacing:1px;color:#7a9cc8;text-shadow:none}
...
.hr i{display:block;font:700 7.5px Arial;font-style:normal;letter-spacing:1px;color:#7a9cc8}
...
.gw{position:relative;border:2px solid rgba(88,199,255,.3);border-radius:14px;overflow:hidden;background:#000;box-shadow:0 0 18px rgba(88,199,255,.15);direction:ltr}
...
.hint{text-align:center;font:600 9.5px Arial;color:#7a9cc8;margin-top:6px}
</style>
<div id="app">
<div class="hdr"><div class="tt">🌀 سونيك داش<small>YATO BOT MD · ANAS MODS</small></div><div class="hrs"><div class="hr"><i>الحلقات</i><b id="rg">0</b></div><div class="hr"><i>النقاط</i><b id="sc">0</b></div><div class="hr"><i>الأفضل</i><b id="bs">0</b></div><button class="mbtn" id="muteB">🔊</button></div></div>
<div class="gw"><canvas id="cv" width="404" height="300"></canvas></div>
<div class="pads"><button class="pd" id="boostB">⚡ انطلاق (Boost)</button><button class="pd" id="jumpB">⤒ قفز (Jump)</button></div>
<div class="hint">اقفز عند وميض خط الليزر الأحمر سريعاً! · كل 50 حلقة = مواجهة الوحش 👹</div>
</div>
<script>
...
sBossHit();popup(boss.x,boss.y-34,'ضربة!','#ffd75e');
...
popup(CX,CY-40,'+25 حلقة!','#ffd75e');
banner={t:0,txt:'تم هزيمة الوحش! +500'};
sBest(.3);
...
if(score>=milestone*500){milestone++;speed=Math.min(9.2,speed+.5);banner={t:0,txt:'زيادة السرعة!'};sMile()}

if(!laser&&!boss&&bossWarnT<=0&&score>700){
laserCd--;
if(laserCd<=0){
laser={t:0,ph:0,y:CY,lockY:0,ex:W+60,fromBoss:false};
sAlarm();popup(W-72,150,'⚠ انتبه لليزر!','#ff5c7a');
...
if(Math.abs(psx-CX)<16&&CY+14>py2-20&&CY<py2+6){vy=-19;grounded=false;wob.v=-.6;qwob.v=-1;sSpring();whirrOn();e2.ph=1;burst(psx,py2-10,7,'#ff5c7a');popup(psx,py2-30,'قفزة عظمى!','#ff5c7a')}
}else if(e2.t==='pad'){
var px2=e2.x-camX;
if(Math.abs(px2-CX)<16&&!e2.used){e2.used=true;boost=Math.min(100,boost+35);boostOn=true;boostT=0;wflash=1;sBoost();humOn();popup(px2,gy(e2.x)-30,'سرعة خارقة!','#ffd75e')}
...
x.lineWidth=6;x.strokeStyle='rgba(20,0,5,.85)';x.strokeText('⚠ خـطـر ⚠',W/2+jx,H/2-14+jy);
...
x.fillStyle=tg;x.fillText('⚠ خـطـر ⚠',W/2+jx,H/2-14+jy);
x.font='bold 11px Arial';x.fillStyle='#ffd0d6';x.fillText('الوحش يقترب — استعد للمواجهة!',W/2,H/2+8);
x.textAlign='left';
}
...
x.fillStyle='#ffd75e';x.font='900 9px Arial';x.textAlign='center';x.fillText('انطلاق »',sx,sy3-21);x.textAlign='left';
...
x.font='bold 8.5px Arial';
if(boostOn){x.fillStyle='#ffd75e';x.fillText('انطلاق!!',bx+Math.sin(frame*.6)*1.5,by+21)}
else if(boost>=25){x.fillStyle='rgba(255,255,255,'+(.55+.45*Math.sin(frame*.25))+')';x.fillText('الانطلاق جاهز!',bx,by+21)}
else{x.fillStyle='rgba(255,255,255,.55)';x.fillText('طاقة الانطلاق',bx,by+21)}
x.fillStyle='rgba(255,255,255,.7)';x.textAlign='right';x.fillText('السرعة '+(speed+(boostOn?4.5:0)).toFixed(1),W-10,20);x.textAlign='left';
...
x.font='bold 8px Arial';x.fillStyle='rgba(255,176,184,.9)';x.textAlign='center';x.fillText('الوحش',W/2,52);x.textAlign='left';
}
...
x.textAlign='center';
x.font='900 26px Arial';x.lineWidth=6;x.strokeStyle='rgba(8,16,40,.9)';x.strokeText('sonic🌀',W/2,108);
var tg4=x.createLinearGradient(0,86,0,112);tg4.addColorStop(0,'#bfe9ff');tg4.addColorStop(.55,'#58c7ff');tg4.addColorStop(1,'#2f7fd6');
x.fillStyle=tg4;x.fillText('sonic🌀',W/2,108);
x.font='bold 11px Arial';x.fillStyle='#ffd75e';x.fillText('ʏᴀᴛᴏ-ʙᴏᴛ ᴀɴᴀs-ᴍᴏᴅs',W/2,126);
if(BEST>0){x.font='bold 11px monospace';x.fillStyle='#58c7ff';x.fillText('الرقم القياسي: '+BEST,W/2,148)}
x.font='900 15px Arial';x.fillStyle='rgba(255,255,255,'+(.55+.45*Math.sin(frame*.09))+')';x.fillText('المس الشاشة للبدء ▶️',W/2,176);
x.font='600 10px Arial';x.fillStyle='#7a9cc8';x.fillText('اقفز لتفادي الليزر الأحمر · كل 50 حلقة = مواجهة الوحش',W/2,198);
x.textAlign='left';
}
...
x.fillStyle=dg3;x.fillText('انتهت اللعبة! 💥',W/2,118);
x.font='bold 12px Arial';x.fillStyle='#eaf2ff';x.fillText('النقاط: '+score+'   ·   الحلقات: '+rings,W/2,144);
if(bestNew){x.fillStyle='rgba(255,215,94,'+(.6+.4*Math.sin(frame*.2))+')';x.font='900 13px Arial';x.fillText('★ رقم قياسي جديد: '+best+' ★',W/2,164)}
else{x.fillStyle='#9fd8ff';x.font='bold 12px Arial';x.fillText('أعلى رقم: '+best,W/2,164)}
x.font='700 11px Arial';x.fillStyle='rgba(255,255,255,'+(.5+.5*Math.sin(frame*.1))+')';x.fillText('المس الشاشة للعب مجدداً 🔄',W/2,188);
x.textAlign='left';
}
}
...
let handler = async (m, { conn }) => {
    try {
        await kirimForwardSigned(conn, m.chat, DASH_HTML, '🌀 لعبة سونيك داش — Anas Mods');
    } catch (e) {
        console.error('[DASH]', e?.message || e);
        m.reply('❌ فشل إرسال اللعبة: ' + (e?.message || e));
    }
};
